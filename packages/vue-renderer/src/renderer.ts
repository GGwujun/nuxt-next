...
export default class VueRenderer {
  serverContext: any;
  options: any;
  renderer: { ssr: undefined; modern: undefined; spa: undefined };
  _state: string;
  _error: null;
  _readyPromise: any;
  // @ts-ignore
  ...
  async _ready() {
    ...
    // -- Development mode --
    if (this.options.dev) {
      this.serverContext.nuxt.hook('build:resources', (mfs: any) =>
  ...
  async loadResources(_fs: {
    exists: (arg0: string) => any;
    readFile: (arg0: string, arg1: any) => any;
  }) {
    const updated = [];

    const readResource = async (fileName: string, encoding: any) => {
      try {
  ...
  renderSSR(renderContext: { modern?: any }) {
  ...
  // @ts-ignore
  async renderRoute(url: string, renderContext = {}, _retried = 0) {
    /* istanbul ignore if */
    if (!this.isReady) {
  ...
  get resourceMap() {
    const publicPath = urlJoin(
      this.options.app.cdnURL || '/',
      this.options.app.assetsPath,
    );
    return {
      clientManifest: {
        fileName: 'client.manifest.json',
        transform: (src: string) =>
          Object.assign(JSON.parse(src), { publicPath }),
      },
      modernManifest: {
      ...
      serverManifest: {
        ...
        transform: async (src: string, { readResource }: any) => {
          const serverManifest = JSON.parse(src);

          const readResources = async (obj: { [x: string]: any }) => {
            const _obj = {};
            await Promise.all(
      ...
      ssrTemplate: {
        ...
        transform: (src: any) => this.parseTemplate(src),
      },
      spaTemplate: {
  ...
  parseTemplate(templateStr: string | undefined) {

...
const _require = require('@vue/cli-shared-utils'),
      info = _require.info,
...
var _default = (api, options) => {
  api.registerCommand('nuxt-serve', {
    description: 'start development server',
    usage: 'vue-cli-service nuxt-serve [options] [entry]',
    options: {
    ...
    var _serve = _asyncToGenerator(function* (args) {
      ...
      const coreInstance = yield service.run({
        name: 'dev',
        args
      });
      ...
      _util.default.initAutomator(args); // although this is primarily a dev server, it is possible that we
      ...
      const isAbsoluteUrl = require('@vue/cli-service/lib/util/isAbsoluteUrl'); // resolve webpack config


      const webpackConfig = api.resolveWebpackConfig(); // check for common config errors

      validateWebpackConfig(webpackConfig, api, options); // load user devServer options with higher priority than devServer
      // in webpck config

      const projectDevServerOptions = Object.assign(webpackConfig.devServer || {}, options.devServer); // expose advanced stats
      ...
      const proxySettings = prepareProxy(projectDevServerOptions.proxy, api.resolve('public')); // inject dev & hot-reload middleware entries

      if (!isProduction) {
        const sockjsUrl = publicUrl ? // explicitly configured via devServer.public
        `?${publicUrl}/sockjs-node` : isInContainer ? // can't infer public netowrk url if inside a container...
        ...
        const devClients = [// dev server client
      ...
      const compiler = webpack(webpackConfig); // create server
      ...
      const server = new WebpackDevServer(compiler, Object.assign({
        ...
        before(app, server) {
          ...
          app.use('/__open-in-editor', launchEditorMiddleware(() => console.log('To specify an editor, sepcify the EDITOR env variable or ' + 'add "editor" field to your Vue project config.\n'))); // allow other plugins to register middlewares, e.g. PWA

          api.service.devServerConfigFns.forEach(fn => fn(app, server)); // apply in project middlewares
      ...
      return new Promise((resolve, reject) => {
        ...
        compiler.hooks.done.tap('vue-cli-service nuxt-serve', stats => {
          if (stats.hasErrors()) {
            return;
          }
          ...
          const printRunningAt = true;
          printRunningAt && console.log();
          printRunningAt && console.log('  App running at:');
          printRunningAt && console.log(`  - Local:   ${chalk.cyan(urls.localUrlForTerminal)} ${copied}`);
          ...
          if (!isInContainer) {
            ...
            console.log(chalk.yellow(`  Access the dev server via ${chalk.cyan(`${protocol}://localhost:<your container's external mapped port>${options.publicPath}`)}`));
          }
